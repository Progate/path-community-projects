...
1. カスタムログフォーマットの定義
    ...
    以下は、レスポンスタイムを含むカスタムログフォーマットの例です。この例では、フォーマットに `request_time` を含め、それを `ltsv` という名前で定義しています。
...
ツールなどを使って負荷を掛ける方法もありますが、今回はシンプルにたくさんリロードしまくって数値の動きやログを見てみましょう。

CPU 順に並び替えて負荷を掛けると、Nginx や PHP-FPM などのプロセスが CPU を多く消費していることがわかるかもしれません。
